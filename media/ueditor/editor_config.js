...
(function () {
    ...
    window.UEDITOR_CONFIG = {
        ...
        //图片上传配置区
        ,imageUrl:ServerURL+"imageUp"           //图片上传提交地址
        ,imagePath:ServerImgURL + ""                     //图片修正地址，引用了fixedImagePath,如有特殊需求，可自行配置
        //,imageFieldName:"upfile"                   //图片数据的key,若此处修改，需要在后台对应文件修改对应参数
        //,compressSide:0                            //等比压缩的基准，确定maxImageSideLength参数的参照对象。0为按照最长边，1为按照宽度，2为按照高度
        //,maxImageSideLength:900                    //上传图片最大允许的边长，超过会自动等比缩放,不缩放就设置一个比较大的值，更多设置在image.html中

        //涂鸦图片配置区
        ,scrawlUrl:URL+"scrawlUp"           //涂鸦上传地址
        ,scrawlPath:URL+""                       //图片修正地址，同imagePath

        //附件上传配置区
        ,fileUrl:ServerURL+"fileUp"               //附件上传提交地址
        ,filePath:ServerImgURL + ""                   //附件修正地址，同imagePath
        //,fileFieldName:"upfile"                    //附件提交的表单名，若此处修改，需要在后台对应文件修改对应参数

         //远程抓取配置区
        ,catchRemoteImageEnable:true               //是否开启远程图片抓取,默认开启
        ,catcherUrl:ServerURL +"getRemoteImage"   //处理远程图片抓取的地址
        ,catcherPath:ServerImgURL + ""                  //图片修正地址，同imagePath
        //,catchFieldName:"upfile"                   //提交到后台远程图片uri合集，若此处修改，需要在后台对应文件修改对应参数
        //,separater:'ue_separate_ue'               //提交至后台的远程图片地址字符串分隔符
        //,localDomain:[]                            //本地顶级域名，当开启远程图片抓取时，除此之外的所有其它域名下的图片都将被抓取到本地

        //图片在线管理配置区
        ,imageManagerUrl:ServerURL + "imageManager"       //图片在线管理的处理地址
        ,imageManagerPath:ServerImgURL + ""                                    //图片修正地址，同imagePath

        //屏幕截图配置区
        ,snapscreenHost: '127.0.0.1'                                  //屏幕截图的server端文件所在的网站地址或者ip，请不要加http://
        ,snapscreenServerUrl: ServerURL +"imageUp" //屏幕截图的server端保存程序，UEditor的范例代码为“URL +"server/upload/php/snapImgUp.php"”
        ,snapscreenPath: ServerImgURL + ""
        //,snapscreenServerPort: 80                                    //屏幕截图的server端端口
        //,snapscreenImgAlign: 'center'                                //截图的图片默认的排版方式


        //word转存配置区
        ,wordImageUrl:ServerURL + "imageUp"             //word转存提交地址
        ,wordImagePath:ServerImgURL + ""                       //
        //,wordImageFieldName:"upfile"                     //word转存表单名若此处修改，需要在后台对应文件修改对应参数


        //获取视频数据的地址
        ,getMovieUrl:ServerURL+"getMovie"                   //视频数据获取地址



        //工具栏上的所有的功能按钮和下拉框，可以在new编辑器的实例时选择自己需要的从新定义
        ,toolbars:[["FullScreen","Source","Undo","Redo","InsertUnorderedList","InsertOrderedList","Unlink","Link","ClearDoc","SelectAll","SearchReplace","Preview","Help","|","GMap","InsertImage","SnapScreen","Emotion","InsertVideo","Attachment","InsertFrame","Date","Time","WordImage","Map","Horizontal","Anchor","Spechars","BlockQuote","HighlightCode","template","background","Bold","Italic","Underline","StrikeThrough","ForeColor","BackColor","Superscript","Subscript","JustifyLeft","JustifyCenter","JustifyRight","JustifyJustify","touppercase","tolowercase","DirectionalityLtr","DirectionalityRtl","Indent","RemoveFormat","FormatMatch","AutoTypeSet","PastePlain","LineHeight","FontFamily","FontSize","ImageNone","ImageLeft","ImageRight","ImageCenter","InsertTable","DeleteTable","MergeRight","MergeDown","SplittoRows","SplittoCols","SplittoCells","MergeCells","InsertCol","InsertRow","DeleteCol","DeleteRow","InsertParagraphBeforeTable"]]
        ...
        ,initialContent:"在这里编辑内容"    //初始化编辑器的内容,也可以通过textarea/script给值，看官网例子
